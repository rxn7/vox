...
struct Default3DVertex {
	constexpr Default3DVertex(vec3 position, vec3 normal, vec2 uv) : position(position), normal(normal), uv(uv) {}
	...
	inline static void setup_attributes() {
		...
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Default3DVertex), (void*)offsetof(Default3DVertex, normal));

		glEnableVertexAttribArray(2);
		glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(Default3DVertex), (void*)offsetof(Default3DVertex, uv));
	}
